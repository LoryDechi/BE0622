...
public class GodfatherPizzaApplication {
	...
	public static void menu(List<Pizza> p, List<Gadget> g, List<Topping> t, List<Bibita> b) {
		...
		System.out.println(String.format("%1$-"+ 15 + "s","Nome") + String.format("%1$-"+ 10 + "s","Calorie") + String.format("%1$-"+ 10 + "s","Prezzo"));
		for ( Pizza pi : p) {
	...
	public static void ordinazione() {
		...
		int scelta = 0;
		do {
			System.out.println("Digita il nome del prodotto da ordinare del cliente " + (scelta+1) + " :");
		...
		switch(risposta) {
			case 1:
				System.out.println("Digita il nome del prodotto da ordinare del cliente" + scelta + " :");
				String prod = input.nextLine();
				Prodotto pr = (Prodotto) ctx.getBean(prod);
				listaProdotti.add(pr);
				break;
		...
		Ordine ordine = (Ordine) ctx.getBean("ordine", ran, t, numeroC, listaProdotti, StatoOrdine.IN_CORSO, LocalTime.now());
		
		System.out.println("Il vostro ordine n°: " + ordine.getNumeroOrdine() + " è in preparazione. Il costo totale del vostro ordine è: " + ordine.getTotale() + "€");
		System.out.println(ordine.getCostoCoperto());
	}
}
